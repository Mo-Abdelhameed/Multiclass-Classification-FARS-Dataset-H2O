...
def save_dataframe_as_csv(dataframe: h2o.H2OFrame, file_path: str) -> None:
    """
    Saves an h2o dataframe to a CSV file in the given directory path.
    Float values are saved with 4 decimal places.

    Args:
    - df (h2o.H2OFrame): The H2O dataframe to be saved.
    ...
    if not os.path.exists(paths.PREDICTIONS_DIR):
        os.makedirs(paths.PREDICTIONS_DIR)
    try:
        h2o.export_file(dataframe, file_path, force=True)
